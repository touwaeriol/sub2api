...
onMounted(() => {
  // window 检测放在 onMounted 内，避免 SSR 环境下 ReferenceError
  checkMobile()
...
<style scoped>
/*
 * 与 host frontend/src/components/layout/TablePageLayout.vue 的关键差异:
 *   host 版本使用 height: calc(100vh - 64px - 4rem) 适配 AppLayout 的 header + padding;
 *   plugin 在 ShadowRoot 内被宿主页面再嵌一层, 那个绝对高度会把表格推出可视区,
 *   因此改用 height: 100% + min-height: 0, 让父级 (PluginPageLayout 或宿主) 决定高度.
 */
.table-page-layout {
