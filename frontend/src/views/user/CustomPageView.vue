...
const menuItem = computed(() => {
  ...
  // For admin users, also search admin-only items from admin settings store
  const adminItems = authStore.isAdmin ? (adminSettingsStore.customMenuItems ?? []) : []
  // Merge: admin items may contain all items (including user-visible), so deduplicate by id
