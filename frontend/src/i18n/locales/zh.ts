export default {
  // 品牌字符串 — 留空则继承 admin 里的 site_name / site_subtitle 设置
  brand: {
    name: '',
    tagline: ''
  },
  ...
  seo: {
    home: {
      title: 'Claude — 所有前沿模型 · 一个 API',
      description:
        '一个密钥对接 Claude、GPT、Gemini 等所有前沿模型。按实际消耗的 token 计费 —— 无需订阅、无需管理多个账号。Anthropic 与 OpenAI SDK 无缝替换。',
      keywords:
        'Claude API,Anthropic API,Claude Opus 4,Claude Sonnet 4,Claude Haiku,1M 上下文,OpenAI API,GPT-4,GPT-5,Gemini API,AI API 网关,统一 API,Claude 中转,Claude 代理,Anthropic 兼容,OpenAI 兼容,按量计费,AI relay,sub2api'
    }
  },
  ...
  home: {
    ...
    goToDashboard: '进入控制台',
    // 新增：面向用户的价值主张
    heroSubtitle: '用前沿智能构建产品。',
    heroDescription: '一个 API，对接所有前沿模型。按实际用量计费 —— 无需订阅、无需管理账号。',
    eyebrow: {
      platform: 'AI 平台',
      capabilities: '能力',
      models: '模型',
    ...
    editorial: {
      capabilitiesTitle: '与大模型协作，所需尽在于此。',
      modelsTitle: '一个 API。\n所有前沿模型。',
      docsTitle: '几分钟内开始开发。',
      ctaTitle: '立即开始构建。',
      ctaBody: '一个密钥，对接所有前沿模型，按用量计费。你需要的都在这里，你不需要的一个也没有。'
    },
    nav: {
      meetClaude: '了解 Claude',
      platform: '平台',
      solutions: '方案',
      capabilities: '能力',
      models: '模型',
    ...
    heroRightTitle: '所有前沿模型\n一个 API 接入',
    trust: {
      sdk: 'Anthropic SDK 兼容',
      openai: 'OpenAI 格式兼容',
      dropin: '无需改代码,直接替换'
    },
    tags: {
    ...
    pricing: {
      ...
      plans: {
        free: {
          ...
          price: '免费',
          desc: '适合个人体验和测试',
          features: ['每日有限额度', '基础模型访问', '社区支持', 'API 密钥 x1']
        },
        pro: {
          name: '专业版',
          price: '¥99',
          desc: '适合个人开发者和重度用户',
          features: ['大额度按量计费', '全部模型访问', '优先支持', 'API 密钥 x5', '用量统计面板']
        },
        team: {
          name: '团队版',
          price: '定制',
          desc: '适合团队和企业客户',
          features: ['不限额度', '全部模型访问', '专属客服', '无限 API 密钥', '团队管理', 'SLA 保障']
        }
      }
    },
    whyChoose: {
      eyebrow: '为什么选我们',
      title: '你需要的都在这里，你不需要的一个也没有。',
      subtitle: '为每天都在和前沿模型打交道的开发者打造。',
      items: {
        pricing: {
          title: '实惠价格',
          desc: '相当于订阅的价格，但只为你真正用掉的 token 付费 —— 比官方直接计费友好得多。'
        },
        stable: {
          title: '稳定可靠',
          desc: '企业级架构下的多节点负载均衡，自动故障转移，保证你的业务不掉线。'
        },
        instant: {
          title: '即时激活',
          desc: '付款即到账，无需人工审核、无需等待，立刻开始调用。'
        },
        models: {
          title: '覆盖所有前沿模型',
          desc: 'Claude、GPT、Gemini 等主流模型 —— 一个密钥全部搞定，随时切换。'
        },
        latency: {
          title: '低延迟智能路由',
          desc: '为每个请求自动挑选最快的上游节点，让你的应用响应飞快。'
        },
        support: {
          title: '真人技术支持',
          desc: '工程师在线待命。出问题时，人工响应以小时计，不是以天计。'
        }
      }
    },
    channels: {
      eyebrow: '定价',
      title: '按你的业务量挑选合适的渠道。',
      subtitle: '通过不同的上游渠道以不同倍率路由。全部以真实美元额度计费，永不过期。',
      viewAll: '查看全部渠道',
      currentRate: '当前倍率',
      creditLine: '1 元 ≈ {usd} 美元额度',
    ...
    faq: {
      eyebrow: '常见问题',
      title: '常见问题解答。',
      subtitle: '这里是我们被问得最多的问题。没找到想问的？随时联系我们。',
      items: {
        buy: {
          q: '如何购买额度？',
          a: '登录后进入充值页面，选择方案并通过支付宝、微信或 Stripe 付款，额度会在几秒内到账。'
        },
        activate: {
          q: '怎么开始使用 API key？',
          a: '注册成功后会生成一个以 sk- 开头的密钥。直接替换上游平台的 base URL 即可使用，无需改动任何 SDK。完整接入指南见文档。'
        },
        expiry: {
          q: '额度会过期吗？',
          a: '不会。额度永不过期，需要时随时充值，账户里会一直累积。'
        },
        models: {
          q: '支持哪些 AI 模型？',
          a: '支持全部 Claude 主力模型（Opus / Sonnet / Haiku，含 1M 上下文版本）、GPT-4 / 4o / 5、Gemini 2.x，以及 Codex 等上游专用模型。新模型会在上游发布后第一时间跟进。'
        },
        refund: {
          q: '可以退款吗？',
          a: '可以。购买后 7 天内未使用的额度支持退款。发邮件给我们带上订单号，24 小时内处理完毕。'
        },
        support: {
          q: '如何获取技术支持？',
          a: "发邮件到 {'hi@anthropic.mom'}，或者使用站内的在线聊天。工作日多数问题会在几小时内得到答复。"
        }
      }
    },
    // 文档模块
    docs: {
      ...
      quickStart: {
        title: '快速开始',
        desc: '5 分钟完成接入，获取你的第一个 API 响应'
      },
      apiRef: {
        title: 'API 参考',
        desc: '完整的 API 接口文档，兼容 OpenAI 格式'
      },
      models: {
        title: '模型列表',
        desc: '查看所有可用模型及其能力和定价'
      },
      faq: {
        title: '常见问题',
        desc: '使用过程中的常见问题和解答'
