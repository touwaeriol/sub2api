...
import { useMenuRegistry } from '@/plugins'
import type { DynamicMenuItem } from '@/plugins'
...
function mapPluginMenuItem(item: DynamicMenuItem): NavItem {
  return {
    path: item.path,
...
const pluginMenuRegistry = useMenuRegistry()
...
const userNavItems = computed((): NavItem[] => {
  const items: NavItem[] = [
    ...
    ...pluginMenuRegistry.getVisible('user').map(mapPluginMenuItem),
  ]
...
const adminNavItems = computed((): NavItem[] => {
  ...
  if (authStore.isSimpleMode) {
    ...
    // append plugin admin menus (simple mode)
    for (const pm of pluginMenuRegistry.getVisible('admin')) {
      filtered.push(mapPluginMenuItem(pm))
    }
    return filtered
  }
  ...
  // append plugin admin menus to regular branch
  for (const pm of pluginMenuRegistry.getVisible('admin')) {
    baseItems.push(mapPluginMenuItem(pm))
  }
  return baseItems
})
