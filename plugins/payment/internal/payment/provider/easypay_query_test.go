...
func TestEasyPayQueryOrderStatusMapping(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantAmount:  decimal.RequireFromString("12.34"),
		},
		{
		...
		},
		{
			...
			wantAmount:  decimal.RequireFromString("9.99"),
		},
		{
			...
			wantStatus:  payment.ProviderStatusPaid,
			wantTradeNo: orderID,
			wantAmount:  decimal.RequireFromString("3.21"),
		},
		{
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if !resp.Amount.Equal(tt.wantAmount) {
