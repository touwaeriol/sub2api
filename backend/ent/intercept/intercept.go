...
// The ProtocolFunc type is an adapter to allow the use of ordinary function as a Querier.
type ProtocolFunc func(context.Context, *ent.ProtocolQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f ProtocolFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.ProtocolQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.ProtocolQuery", q)
}

// The TraverseProtocol type is an adapter to allow the use of ordinary function as Traverser.
type TraverseProtocol func(context.Context, *ent.ProtocolQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseProtocol) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseProtocol) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.ProtocolQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.ProtocolQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.ProtocolQuery:
		return &query[*ent.ProtocolQuery, predicate.Protocol, protocol.OrderOption]{typ: ent.TypeProtocol, tq: q}, nil
	case *ent.ProxyQuery:
