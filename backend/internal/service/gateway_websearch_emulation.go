...
// Account-level mode: "enabled" (force on), "disabled" / "default" (force off).
//
// NOTE: v0.1.115 上游版本的 default 分支会查 channelService.GetChannelForGroup
// 然后 ch.IsWebSearchEmulationEnabled(platform) 让 channel 一键开关 web search emulation。
// fork 把渠道管理迁到 plugin，host 不再持有 channelService/Channel struct，所以这里
// 退化为"账号 extra.web_search_emulation 显式 enable 才启用"的语义（方案 A）。
// V5-CURATE §X：如果将来要恢复 channel 级开关，扩 ChannelExtension 加 RPC，host 反向查 plugin。
func (s *GatewayService) shouldEmulateWebSearch(ctx context.Context, account *Account, groupID *int64, body []byte) bool {
	_ = groupID // channel-level emulation moved to plugin (V5 W7); groupID retained for signature stability
	...
	case WebSearchModeDisabled:
		return false
	default: // "default" → channel-level emulation moved to plugin (V5 W7); fall through to disabled
		return false
	}
}
