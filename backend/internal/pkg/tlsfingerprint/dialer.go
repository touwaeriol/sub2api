...
func maybeEnrichExtensions(extOrder []uint16) []uint16 {
	...
	// Insert 0xfe0d at index 1 (after server_name) and append 21 at end.
	if len(extOrder) < 1 || extOrder[0] != 0 {
		// Defensive: if the profile's extension list doesn't start with
		// server_name, don't touch it — caller-supplied ordering wins.
		return extOrder
	}
...
func buildClientHelloSpecFromProfile(profile *Profile) *utls.ClientHelloSpec {
	...
	// Determine extension order. For the default order (no profile override),
	// roll a per-connection probabilistic decision to emit the "rich" variant
	// (ECH + padding) vs the "plain" variant — matches real Claude Code 2.1.114
	// which alternates JA3 hashes ~1:1 based on this flag. Profile-supplied
	// orders are respected as-is.
	var extOrder []uint16
