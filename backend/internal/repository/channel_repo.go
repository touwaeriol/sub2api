...
func (r *channelRepository) Create(ctx context.Context, channel *service.Channel) error {
	return r.runInTx(ctx, func(tx *sql.Tx) error {
		...
		paramOverridesJSON, err := marshalParamOverrides(channel.ParamOverrides)
		if err != nil {
			return err
		}
		err = tx.QueryRowContext(ctx,
			`INSERT INTO channels (name, description, status, model_mapping, billing_model_source, restrict_models, features, param_overrides) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
			 RETURNING id, created_at, updated_at`,
			channel.Name, channel.Description, channel.Status, modelMappingJSON, channel.BillingModelSource, channel.RestrictModels, channel.Features, paramOverridesJSON,
...
func (r *channelRepository) GetByID(ctx context.Context, id int64) (*service.Channel, error) {
	ch := &service.Channel{}
	var modelMappingJSON, paramOverridesJSON []byte
	err := r.db.QueryRowContext(ctx,
		`SELECT id, name, description, status, model_mapping, billing_model_source, restrict_models, features, param_overrides, created_at, updated_at
		 FROM channels WHERE id = $1`, id,
	).Scan(&ch.ID, &ch.Name, &ch.Description, &ch.Status, &modelMappingJSON, &ch.BillingModelSource, &ch.RestrictModels, &ch.Features, &paramOverridesJSON, &ch.CreatedAt, &ch.UpdatedAt)
	...
	ch.ParamOverrides = unmarshalParamOverrides(paramOverridesJSON)
...
func (r *channelRepository) Update(ctx context.Context, channel *service.Channel) error {
	return r.runInTx(ctx, func(tx *sql.Tx) error {
		...
		result, err := tx.ExecContext(ctx,
			`UPDATE channels SET name = $1, description = $2, status = $3, model_mapping = $4, billing_model_source = $5, restrict_models = $6, features = $7, param_overrides = $8, updated_at = NOW()
			 WHERE id = $9`,
			channel.Name, channel.Description, channel.Status, modelMappingJSON, channel.BillingModelSource, channel.RestrictModels, channel.Features, paramOverridesJSON, channel.ID,
		)
		if err != nil {
			if isUniqueViolation(err) {
				return service.ErrChannelExists
...
func (r *channelRepository) List(ctx context.Context, params pagination.PaginationParams, status, search string) ([]service.Channel, *pagination.PaginationResult, error) {
	...
	dataQuery := fmt.Sprintf(
		`SELECT c.id, c.name, c.description, c.status, c.model_mapping, c.billing_model_source, c.restrict_models, c.features, c.param_overrides, c.created_at, c.updated_at
	...
	for rows.Next() {
		...
		var modelMappingJSON, paramOverridesJSON []byte
		if err := rows.Scan(&ch.ID, &ch.Name, &ch.Description, &ch.Status, &modelMappingJSON, &ch.BillingModelSource, &ch.RestrictModels, &ch.Features, &paramOverridesJSON, &ch.CreatedAt, &ch.UpdatedAt); err != nil {
...
func (r *channelRepository) ListAll(ctx context.Context) ([]service.Channel, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, name, description, status, model_mapping, billing_model_source, restrict_models, features, param_overrides, created_at, updated_at FROM channels ORDER BY id`,
	)
	if err != nil {
...
// marshalParamOverrides 将渠道参数覆盖规则序列化为 JSON 字节。
// nil 或空 map 返回 "{}" 以匹配 channels.param_overrides 的 NOT NULL 默认值。
func marshalParamOverrides(o service.ChannelParamOverrides) ([]byte, error) {
	if len(o) == 0 {
		return []byte("{}"), nil
	}
	data, err := json.Marshal(o)
	if err != nil {
		return nil, fmt.Errorf("marshal param_overrides: %w", err)
	}
	return data, nil
}

// unmarshalParamOverrides 将 JSON 字节反序列化为渠道参数覆盖规则。
// 非法 JSON 返回 nil（handler 层会兜底为 {}），与 unmarshalModelMapping 保持一致。
func unmarshalParamOverrides(data []byte) service.ChannelParamOverrides {
	if len(data) == 0 {
		return nil
	}
	var o service.ChannelParamOverrides
